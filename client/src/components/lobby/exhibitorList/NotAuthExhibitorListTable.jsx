...
const NotAuthExhibitorListTable = ({ options }) => {
  const {
    columns,
    data = [],
    isPaginating,
  ...
  const isAllData = data.length === totalSize;
  const pageNums = isAllData
    ? Math.ceil(data.length / baseNumber)
    : Math.ceil(totalSize / baseNumber);
  ...
  const updateData = async (index) => {
    if (!isAllData) await onPaginationClick(`pageNo=${index}`);
  ...
  useEffect(() => {
    if (isPaginating && data.length) {
      ...
      if (isAllData) {
        // exhibitor 와 network 구분
        setSlicedData(data.slice(offset, offset + baseNumber));
      }
    }
  }, [data, offset]);
      ...
      <div className="tb-wrap">
        <table className="center">
          ...
          <tbody>
            ...
            ) : isPaginating && isAllData ? (
              slicedData.map((v, index) => renderRow(v, index))
            ) : (
...
NotAuthExhibitorListTable.propTypes = {
...
export default NotAuthExhibitorListTable;
